...
class window_process(threading.Thread):
	...
	def update(self): #update the window
		ret, frame = self.cam_stream.read()
...
robot_arm = usb()
session = cv_module()
cam_view = window_process(ready,session.vid_cap_device)
...
while (True): #main routine
	if(ready):
		ret, frame = cam_view.cam_stream.read()
		...
		if not circle_gen:
			...
			# loop over the (x, y) coordinates and radius of the circles
				...
				# corresponding to the center of the circle
					cv2.circle(cam_view.overlay, (x, y), r, (0, 255, 0), 4)
					if (r > 0) and (x > 0) and (y > 0):
						xyr[0] = int(100-100*x/cam_view.window_W)
						xyr[1] = int(100-100*y/cam_view.window_H)
						xyr[2] = int(100*r/480)
						cvxyr = x,y,r
						origin = [xyr[0],xyr[1]]
						increment = 40
